...
namespace Microsoft.Azure.IIoT.OpcUa.Vault.KeyVault {
    ...
    public class KeyVaultServiceClient {
        ...
        public async Task<string> GetCertificateConfigurationGroupsAsync(CancellationToken ct = default) {
            var secret = await _keyVaultClient.GetSecretAsync(_vaultBaseUrl, _groupSecret, ct).ConfigureAwait(false);
            return secret.Value;
        }
        ...
        public async Task<string> PutCertificateConfigurationGroupsAsync(string json, CancellationToken ct = default) {
            var secret = await _keyVaultClient.SetSecretAsync(_vaultBaseUrl, _groupSecret, json, null, ContentTypeJson, null, ct).ConfigureAwait(false);
            return secret.Value;
        }
        ...
        public async Task<Models.KeyVaultTrustListModel> GetTrustListAsync(string id, int? maxResults, string nextPageLink, CancellationToken ct = default) {
            ...
            while (secretItems != null) {
                foreach (var secretItem in secretItems.Where(s => s.Tags != null)) {
                    var tag = secretItem.Tags.FirstOrDefault(x => string.Equals(x.Key, id, StringComparison.OrdinalIgnoreCase)).Value;
                    var issuer = tag == TagIssuerList;
                    var trusted = tag == TagTrustedList;
                    var certType = secretItem.ContentType == ContentTypeCert;
                    var crlType = secretItem.ContentType == ContentTypeCrl;
            ...
            while (certItems != null) {
                foreach (var certItem in certItems.Where(c => c.Tags != null)) {
                    var tag = certItem.Tags.FirstOrDefault(x => string.Equals(x.Key, id, StringComparison.OrdinalIgnoreCase)).Value;
                    var issuer = tag == TagIssuerList;
                    var trusted = tag == TagTrustedList;
